"""T126 (AL-7): toda mutacion de `/api/v1/packages/**` debe rechazar un
`Origin` cruzado o ausente, y `resume`/`undo` deben exigir `package_hash`
igual que `approve`/`reject` (mismo patron anti-CSRF de contenido: sin la
huella viva, ni siquiera un ataque CSRF con cookie valida puede cambiar
nada sin conocer el estado exacto del paquete)."""
...
from safent_ads.composition.api import PackagesOriginRequiredMiddleware, _handle_api_error
    ...
    async with container.session_factory() as session:
        await session.execute(
            text(
                ...
                "VALUES (:id, :slug, 'Negocio CSRF', 'Europe/Madrid', 'EUR')"
            ),
            {"id": business_id, "slug": f"pkg-csrf-{uuid.uuid4().hex[:10]}"},
        )
...
def _app(container: Container, asset_store: LocalAssetStorage) -> FastAPI:
    ...
    application.add_middleware(PackagesOriginRequiredMiddleware)
...
async def test_post_sin_origin_es_403(
    ...
    async with _client(seeded.container, asset_store, authenticated_session.cookies) as client:
        # httpx no manda `Origin` por defecto -- exactamente la peticion
        # cross-site que un formulario/fetch de otro origen produciria con
        # la cookie de sesion todavia valida en el navegador de la victima.
...
async def test_undo_sin_package_hash_es_422(
    seeded: _Seeded, authenticated_session: AuthenticatedSession, tmp_path: Path
) -> None:
    package = await _seed_package(seeded.container, seeded.scope)
    asset_store = LocalAssetStorage(tmp_path, b"0" * 32, SystemClock())
    ...
    async with _client(seeded.container, asset_store, authenticated_session.cookies) as client:
        response = await client.post(
            f"/api/v1/packages/{package.package_id}/undo?business_id={seeded.scope.business_id}",
...
async def test_resume_sin_package_hash_es_422(
    seeded: _Seeded, authenticated_session: AuthenticatedSession, tmp_path: Path
) -> None:
    package = await _seed_package(seeded.container, seeded.scope)
    asset_store = LocalAssetStorage(tmp_path, b"0" * 32, SystemClock())
