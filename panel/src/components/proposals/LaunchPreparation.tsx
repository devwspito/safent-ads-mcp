...
export function LaunchPreparation({ plan, businessId }: { plan: LaunchPlan; businessId: string }) {
  ...
  return <section className={styles.preparation} aria-label="Preparación y runtime">
    ...
    <details className={styles.connector}>
      ...
      <p>Instalación recomendada en cada equipo (macOS / Linux): registra el MCP, abre la autorización y vincula el conector sin copiar claves. El parámetro --background instala un servicio de usuario y autoriza el consumo de cuota.</p>
      <pre className={styles.command}>{`uv run python -m safent_ads.runtime.installation install --url ${window.location.origin} --runtime ${runtime} --background`}</pre>
      <p>Añadir sólo la URL MCP no instala el conector local. Ejecuta el instalador desde el repositorio del motor con el runtime ya instalado y autenticado.</p>
